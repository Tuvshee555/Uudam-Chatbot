...
export function buildPromptParts(options: BuildPromptOptions): { system: string; user: string } {
  ...
  lines.push("- Example ambiguity reply (replace <хот> with the ACTUAL destination the user asked about — never print the word 'чиглэл' or brackets literally): '<хот> чиглэлд 3 өөр аялал байна — та алийг нь сонирхож байна вэ? 😊' then each trip on its own line: '✈️ Шууд нислэгтэй — 6 өдөр', '🚌 Газрын — 8 өдөр', '✈️🚌 Газар+нислэг хосолсон — 9 өдөр', with one BUTTONS entry per trip.");
