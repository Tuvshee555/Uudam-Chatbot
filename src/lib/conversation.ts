...
export function buildPromptParts(options: BuildPromptOptions): { system: string; user: string } {
  ...
  // Truth rules, each one added after a real customer was given a wrong answer
  // (found reading real conversations, 2026-09-20).
  lines.push("- AGE → FARE: use ONLY the age bands written in that trip's own data. An infant (нярай) is a child under 2 years (0-23 сар / months) unless the data says otherwise; a child aged 2 or older is NEVER charged the infant fare (a 4-year-old was once told to pay the infant price). If the data has no band that covers the child's age, do not guess a tier — REFER.");
  lines.push("- NEVER CONFIRM WHAT IS NOT WRITTEN: do not answer 'Тийм ээ' to a claim about what a trip includes, how many days are spent in a city, which cities or airports it visits, or what a place is like, unless that exact fact is written in that trip's data. If it is not written, output exactly REFER instead of agreeing or denying (the system then stays silent to the customer and alerts staff).");
  lines.push("- NEVER DESCRIBE THE WHOLE CATALOG FROM MEMORY: do not use words like 'all', 'most', 'usually', 'only' about our trips as a group (no 'all our trips are 7-10 days', no 'we do not have X'). For 'is there a shorter / cheaper / longer trip', find the single shortest / cheapest / longest matching trip in Context and answer with that trip's name and its real duration or price; if several tie, name them.");
  lines.push("- VAGUE MESSAGE → ASK, DO NOT ASSUME: when the message is a single vague word or does not clearly name a trip or destination (for example 'Сонгоцон'), do NOT pick a trip for the customer and do NOT quote a price or date. Ask which trip or destination they mean.");
  ...
  lines.push("- Example ambiguity reply (replace Бээжин with the ACTUAL destination the user asked about — never print the word 'чиглэл' or brackets literally): 'Бээжин чиглэлд 3 өөр аялал байна — та алийг нь сонирхож байна вэ? 😊' then each trip on its own line: '✈️ Шууд нислэгтэй — 6 өдөр', '🚌 Газрын — 8 өдөр', '✈️🚌 Газар+нислэг хосолсон — 9 өдөр', with one BUTTONS entry per trip.");
