...
const CATALOG: TripLike[] = [
  { route_name: "Бээжин шууд нислэгтэй аялал", adult_price: 1210000, child_price: 1170000 },
  { route_name: "Бээжин галт тэрэгний аялал", adult_price: 1490000, child_price: 1130000 },
  { route_name: "Хайнан Саньяа аялал", adult_price: 3200000, child_price: 2800000 },
  { route_name: "Далянь амралт", adult_price: 1250000, child_price: 1800000 },
];
...
test("fires when the reply prices a different destination than the one asked about", () => {
  const leak = findWrongTripReference({
    replyText: "Бээжин шууд нислэгтэй аялал: том хүн 1,210,000₮, хүүхэд 1,170,000₮.",
...
test("stays out when no specific trip was resolved (broad / list / recommend question)", () => {
  ...
  const leak = findWrongTripReference({
    replyText: "Одоогоор 3 аялал байна: Бээжин 1,210,000₮, Хайнан 3,200,000₮, Далянь 1,250,000₮.",
    relevantTripNames: [],
    catalog: CATALOG,
  });
...
test("does not fire on a compare answer that names both relevant trips", () => {
  const leak = findWrongTripReference({
    replyText:
      "Бээжин шууд нислэгтэй аялал 1,210,000₮, Хайнан Саньяа аялал 3,200,000₮ — Бээжин хямд.",
...
test("does not fire when the reply is on-topic (mentions the asked-for trip)", () => {
  const leak = findWrongTripReference({
    replyText: "Далянь амралт: том хүн 1,250,000₮, хүүхэд 1,800,000₮.",
    relevantTripNames: ["Далянь амралт"],
    catalog: CATALOG,
  });
  assert.equal(leak, null);
...
test("does not fire when the reply prices the asked trip AND mentions another in passing", () => {
  ...
  const leak = findWrongTripReference({
    replyText: "Далянь амралт 1,250,000₮. Хайнан ч бас байгаа, сонирхвол хэлээрэй.",
    relevantTripNames: ["Далянь амралт"],
    catalog: CATALOG,
  });
  assert.equal(leak, null);
